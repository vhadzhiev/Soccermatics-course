...
# only accurate/successful passes made by a team until the first substitution
# check for index of first sub
sub = df.loc[df["type_name"] == "Substitution"].loc[df["team_name"] == "Portugal"].iloc[0]["index"]
# make df with successfull passes by Portugal until the first substitution
mask_portugal = (df.type_name == 'Pass') & (df.team_name == "Portugal") & (df.index < sub) & (
    df.outcome_name.isnull()) & (df.sub_type_name != "Throw-in")
# taking necessary columns
df_pass = df.loc[mask_portugal, ['x', 'y', 'end_x', 'end_y', "player_name", "pass_recipient_name"]]
...
fig.suptitle("Nodes location - Portugal", fontsize=30)
plt.show()
...
fig.suptitle("Portugal Passing Network against Spain", fontsize=30)
plt.show()
